...
- 기존 OCI Email Delivery SMTP를 재사용하고 Mattermost 유료 기능을 활성화하지 않는다.
- 저장소에는 재사용 가능한 기능으로 포함하되 신규 설치에서는 기본 비활성화한다.
...
- 새로운 OCI IAM 사용자, 정책, SMTP 자격 증명, DNS 또는 네트워크 리소스 생성
...
```text
https://threadhub-mentor.stillwhy.com/pl/{post_id}
```
...
- 기존 OCI Email Delivery SMTP 자격 증명을 재사용한다.
- 발신 주소는 기존 OCI Email Delivery Approved Sender를 사용한다.
...
```json
{
  ...
  "permalink": "https://threadhub-mentor.stillwhy.com/pl/mattermost-post-id",
...
새 ThreadHub 설치에서는 이 기능을 기본 비활성화한다.

```text
NOTIFIER_ENABLED=false
NOTIFIER_MODE=allowlist
NOTIFIER_CHANNEL_IDS=
NOTIFIER_ACTIVATED_AT=
```

설치자가 SMTP, 개인정보 처리, 예상 수신자 수 및 비용을 확인한 뒤 명시적으로 활성화해야 한다.

### 8.2 라이브 단계적 활성화

`threadhub-mentor.stillwhy.com`에서는 다음 순서를 지킨다.

1. Mailer를 `NOTIFIER_ENABLED=false`로 배포한다.
2. 플러그인을 설치하되 기존 채널에 전역 발송하지 않는다.
3. 관리자 2명만 있는 임시 Team에 공개·비공개 시험 채널을 만든다.
4. `NOTIFIER_MODE=allowlist`와 시험 채널 ID만 설정한다.
5. `NOTIFIER_ACTIVATED_AT`을 시험 시작 시각으로 설정해 이전 이벤트 발송을 막는다.
6. 승인 시험을 모두 통과한다.
7. 운영자의 명시적 승인 후 공개·비공개 채널 전역 모드로 전환한다.
...
기능은 저장소와 신규 설치에서 기본 비활성화한다. `threadhub-mentor.stillwhy.com`에서는 관리자 전용 시험 채널 allowlist로 검증한 후에만 전역 활성화한다. SMTP 장애는 채팅 기능과 분리하며, 잘못된 수신자나 비공개 정보 노출이 확인되면 즉시 비활성화하고 롤백한다.
