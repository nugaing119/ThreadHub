...
| FR-NOT-004 | 기본 알림은 프로젝트 도메인, Team 표시명, 채널 표시명과 새 글/스레드 답글 유형을 포함하되 메시지 본문, 작성자명과 첨부파일명은 포함하지 않아야 한다. 채널명 자체가 기밀이면 일반형 모드를 선택할 수 있어야 한다. | 필수 |
| FR-NOT-005 | 수신자별로 분리된 SMTP envelope를 사용하고 다른 수신자 주소를 노출하지 않아야 한다. | 필수 |
...
| AC-NOT-004 | 기본 알림에 도메인·Team·채널·새 글/답글 유형이 정확하고, 이메일에 메시지 본문·작성자명·첨부파일명과 다른 수신자 주소가 없다. 일반형 모드에서는 Team·채널명이 없다. |
| AC-NOT-005 | 각 이메일은 수신자별 단일 SMTP envelope를 사용한다. |
