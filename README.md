...
canonical fresh 신규 설치의 기본 알림 대상은 `all_channels`입니다. 활성화 시각 이후
공개·비공개 채널의 새 글과 스레드 답글이 자동 대상이 되고, 이후 생성한 채널도 별도
allowlist 등록 없이 포함됩니다. 실제 수신자는 게시 시점의 해당 채널 멤버로 제한되며
작성자, 비활성 사용자, 봇, DM·그룹 DM과 시스템 글은 제외됩니다. 이 기본값은 SMTP
acceptance와 정확한 plugin·빈 queue 검증을 통과하기 전에는 발송을 시작하지 않습니다.
이미 운영 중인 Mattermost에는 이 기본값을 바로 적용하지 않고 allowlist 파일럿과
운영 책임자의 명시 승인을 거칩니다.

백업은 기본 서비스 설치와 분리된 승인 단계입니다. 최초 수동 백업의 원격 검증과
폐기 가능한 새 VM 복구시험을 마치기 전에는 예약 타이머를 활성화하지 않습니다.
활성화 뒤에는 동일한 systemd service를 직접 한 번 인수하고 다음 예약 실행도 확인합니다.
