...
두 모델이 공통으로 만족해야 하는 기능·보안·알림·백업 목표 상태와 기존 인스턴스
보호 gate는 [CRS-1 표준](./canonical-runtime-standard.md)을 따른다. 배포 모델은
hostname별 특수 구성을 허용하는 분류가 아니라 검증된 두 물리 레이아웃의 구분이다.
