...
`backup-restore-integration` 표는 실제로 통과한 커밋만 기록합니다. 아직 이 브랜치의
Ubuntu real-image 실행 결과를 기록하지 않았으므로 데이터 행은 비워 둡니다. 허용된
필드 외에 버킷, namespace, OCID, 도메인, 이메일, backup ID, object key, 데이터
크기, 사용자 수와 파일명을 공개하지 않습니다.
