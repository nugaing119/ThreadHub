...
### 10.7 비허용 외부 네트워크의 SSH 차단

[Check-Host TCP 검사 API](https://check-host.net/about/api)를 사용해 OCI SSH 허용 목록에 포함되지 않은 외부 8개 노드에서 접속을 시험했다. 검사 지역은 오스트리아, 브라질, 스페인 2곳, 이란, 네덜란드, 러시아와 미국이다.

| 시험 | 결과 | 판정 |
| --- | --- | --- |
| TCP 22 비허용 기준선 | 8개 노드 모두 연결 시간 초과 | 통과 |
| TCP 443 공개 포트 대조군 | 동일 8개 노드 모두 연결, 0.166~0.382초 | 통과 |
| TCP 22 허용 목록 양성 대조군 | 미국 진단 노드 `157.254.174.204/32`를 일시 허용하자 0.234초에 연결 | 통과 |
| 임시 규칙 제거 후 재시험 | 같은 진단 노드에서 다시 연결 시간 초과 | 통과 |

동일 외부 노드에서 공개 HTTPS는 연결되고, SSH는 허용 목록에 있을 때만 연결되며 규칙 제거 후 다시 차단됐다. 임시 진단 규칙이 OCI Security List에서 제거된 것도 확인했으므로 `NET-06`을 통과로 판정한다.
...
- 비관리자 네트워크 8곳에서 SSH 차단, 동일 노드 HTTPS 연결 대조: 통과
