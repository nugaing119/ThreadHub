...
canonical fresh 신규 설치는 `all_channels`를 기본 목표로 사용한다. 이 모드는 activation
cutoff 이후 존재하는 채널뿐 아니라 나중에 생성한 공개·비공개 채널도 자동으로 포함한다.
채널 전체를 대상으로 한다는 뜻은 모든 Team 사용자를 일괄 수신자로 삼는다는 뜻이
아니며, 각 게시 시점의 해당 채널 멤버에게만 발송한다. 과거 글을 소급 발송하지 않고
DM·그룹 DM·시스템 글·Webhook·봇 작성 글은 계속 제외한다.

신규 설치도 SMTP acceptance, 정확한 plugin과 빈 pre-activation queue 검증 전에는
물리적 발송을 시작하지 않는다. 고객 초대 전에 공개·비공개 채널 수동 인수시험을
완료한다. 기존 운영 Mattermost 채택은 데이터와 사용자 활동이 이미 있으므로 이 기본
활성화 경로를 사용하지 않으며, allowlist 파일럿과 운영 책임자의 명시 승인을 거친다.
