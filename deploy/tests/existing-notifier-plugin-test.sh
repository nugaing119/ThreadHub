    ...
    case "${command_name}" in
        ...
        exec)
            ...
            case "${action}" in
                ...
                enable)
                    version="$(jq -er '.version' "${target_root}/plugin.json")" || return 1
                    if [[ "${FIXTURE_FAIL_NEW_ENABLE:-false}" == true && "${version}" == 0.2.1 ]]; then
                        return 91
                    fi
                    printf 'active\n' > "${plugin_state}"
                    ;;
...
NOTIFIER_PLUGIN_BUNDLE=notifier/dist/${plugin_id}-0.2.1.tar.gz
NOTIFIER_PLUGIN_BUNDLE_SHA256=${bundle_sha}
NOTIFIER_MAILER_IMAGE=threadhub/notifier-mailer:0.2.1
...
prepare_fixture() {
    ...
    repository_bundle="${REPOSITORY_ROOT}/notifier/dist/com.threadhub.channel-email-notifier-0.2.1.tar.gz"
    mkdir -p \
    ...
    printf 'missing\n' > "${plugin_state}"
    printf 'enabled\n' > "${plugin_runtime_state}"
    create_reviewed_tree "${reviewed_root}" 0.2.1
